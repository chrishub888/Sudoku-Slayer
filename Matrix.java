			...
			for (int val = curValue; val <= ySize; val++) { // Cycle through values 1-9    
				...
				if (alreadyUsed == 0) {
					...
					if ((x!=xSize-1) || (y!=ySize-1)) { // if both of these tests are False, it means we are at the end of the matrix so stop recursive calls.
						...
						val = ySize+1; // When we're done (last row, last column), force the for loop to exit.
				} // end else	
			} // end for
			if ((alreadyUsed == 1) && ((x < xSize-1) || (y < ySize - 1))) { // if we could not find a solution to a cell, zero it and set direction to go backwards.
				matrix[x][y][0]=0;
				direction = -1;
				return(0);
			}
