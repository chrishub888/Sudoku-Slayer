...
public class Matrix {
	...
	int bruteForce(int curValue, int x, int y) {
		...
		if ((alreadyUsed == 1) && (solved == 0)) { // if we could not find a solution to a cell, zero it and set direction to go backwards.
			matrix[x][y][0]=0;
			direction = -1;
			return(0);
		}
	    ...
	    endTimeMSInMatrix = System.currentTimeMillis();
